...
public int npcUpkeepTicks = AWCoreStatics.npcUpkeepTicks;//how many upkeep ticks worth of food are remaining?

protected int npcHealingTicks = AWCoreStatics.npcHealingTicks;//
  ...
  if(npcType.isCombatUnit() && !npcType.getConfigName().equals(""))
    ...
    if(targets!=null && targets.length>0)
      ...
      for(String name : targets)
        ...
        if(clz!=null)
          {
          targetHelper.addTargetEntry(new AITargetEntry(this, TargetType.ATTACK, clz, 0, true, AWCoreStatics.npcAISearchRange));
          }
        }
      }
    }
...
protected void updateAITick() 
  {
  if(aiTick < AWCoreStatics.npcAITicks || this.isDead)
    {
    aiTick++;
    return;
    }
...
public void onUpdate()
  ...
  if(!worldObj.isRemote && (npcTicksExisted + this.entityId) % AWCoreStatics.npcAITicks == 0)
    {
  ...
  if(this.npcUpkeepTicks!=0)
    ...
    if(npcHealingTicks==0)
      {
      npcHealingTicks = AWCoreStatics.npcHealingTicks;
...
public void onStuckDetected()
  {
  if(!this.worldObj.isRemote && AWCoreStatics.enableNpcTeleportHome && this.wayNav.getHomePoint()!=null && this.getTargetType()==TargetType.SHELTER)
    {    
