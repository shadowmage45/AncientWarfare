...
public void handleMoveData(NBTTagCompound tag)
  ...
  if(tag.hasKey("rp"))
    {
    this.pitchTicks = AWCoreStatics.vehicleMoveUpdateFrequency+1;
    this.destPitch = tag.getFloat("rp");
    }
  if(tag.hasKey("ry"))
    {
    this.rotationTicks = AWCoreStatics.vehicleMoveUpdateFrequency+1;
    this.destYaw = tag.getFloat("ry");
    }
  if(tag.hasKey("px"))
    {
    this.moveTicks = AWCoreStatics.vehicleMoveUpdateFrequency+1;
...
protected void onUpdateServer()
  ...
  sendUpdate = sendUpdate && this.vehicle.ticksExisted % AWCoreStatics.vehicleMoveUpdateFrequency==0;
...
protected void tearUpGrass()
  {
  if(vehicle.worldObj.isRemote || !vehicle.onGround || !AWCoreStatics.vehiclesTearUpGrass)
    {
    return;
    }
