...
public static boolean writeStructureToFile(ProcessedStructure struct, String name)
  ...
  if(!outputFile.exists())
    ...
    try
      ...
      {
      AWLog.logError("Could not create file for structure: "+name);
      e.printStackTrace();
      return false;
      }
    }
    ...
    {
    AWLog.logError("Exporting would overwrite structure with name: "+name+"  Operation aborted.  Please choose a different name before attempting to export again.");
    return false;    
    }
  ...
  try
    ...
    {
    AWLog.logError("Severe error attempting to write structure to export directory!");
    e.printStackTrace();
    }
  return false;
  }
