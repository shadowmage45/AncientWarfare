...
//      Config.logDebug("type: "+ammo.getDisplayName()+" missile count to fire: "+count + " hasSecondaryAmmo: "+ammo.hasSecondaryAmmo() + " secType: "+ammo.getSecondaryAmmoType());
      ...
      for(int i = 0; i < count; i++)
        ...
        if(AWCoreStatics.adjustMissilesForAccuracy)
          {        
...
public void handleAimInput(Vec3 target)
  ...
  if(vehicle.canAimPitch())
    {   
    Pair<Float, Float> angles = TrajectoryCalculator.getLaunchAngleToHit(tx, ty, tz, vehicle.localLaunchPower);  
    ...
    {     
    float power = TrajectoryCalculator.iterativeSpeedFinder(tx, ty, tz, vehicle.localTurretPitch+vehicle.rotationPitch, Settings.getClientPowerIterations(), (vehicle.ammoHelper.getCurrentAmmoType()!=null && vehicle.ammoHelper.getCurrentAmmoType().isRocket()));    
