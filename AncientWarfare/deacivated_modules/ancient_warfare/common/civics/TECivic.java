...
import shadowmage.ancient_framework.common.network.Packet04TE;
...
protected int tickDivider = AWCoreStatics.npcAITicks * 10;
...
protected void onCivicUpdate()
  ...
  this.updateHasWork();
  this.broadcastWork(AWCoreStatics.civicBroadcastRange);
  ...
  for(NpcBase npc : npcList)
    ...
    if(isHostile(npc.teamNum))      
      ...
      if(npc.npcType.isCombatUnit())
        {
        npc.handleTileEntityTargetBroadcast(this, TargetType.ATTACK_TILE, AWCoreStatics.npcAITicks*11);
        }      
      }
    else
      {
      if(broadcastWork)
        ...
        if(hasWork() && canHaveMoreWorkers(npc) && npc.npcType.getWorkTypes(npc.rank).contains(civic.getWorkType()) && npc.teamNum==this.teamNum)
          {
          npc.handleTileEntityTargetBroadcast(this, TargetType.WORK, AWCoreStatics.npcAITicks*11);
          }
        }
      }
    }
...
protected void validateWorkers()
  ...
  while(workIt.hasNext())
    ...
    if(npc.getDistanceFrom(xCoord, yCoord, zCoord)>AWCoreStatics.npcAISearchRange)
      {
      workIt.remove();
      continue;
      }
