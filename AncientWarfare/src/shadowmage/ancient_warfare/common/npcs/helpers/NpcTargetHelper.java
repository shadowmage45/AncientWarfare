...
//  long t = System.nanoTime();
  float mr = (float)AWCoreStatics.npcAISearchRange;
  float dist = 0;
...
//                    Config.logDebug("adding/updating entity aggro entry for target: "+ent);
          ...
          int aggroAmt = (int)(AWCoreStatics.npcAITicks + (distPercent * (float)AWCoreStatics.npcAITicks)); 
...
public void handleBeingAttacked(EntityLivingBase damager)
  ...
  if(this.targetEntries.containsKey(TargetType.ATTACK))
    {
    this.addOrUpdateAggroEntry(revengeEntry, damager, AWCoreStatics.npcAITicks * 4);
    }
  }
...
public void handleBroadcastTarget(Entity ent, TargetType type, int multi)
  ...
  if(this.targetEntries.containsKey(type))
    ...
    if(entry!=null)
      {
      this.addOrUpdateAggroEntry(entry, ent, AWCoreStatics.npcAITicks * multi);
      }
    }
  }
