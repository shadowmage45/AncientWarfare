...
public void updateEntity()
  ...
  if(this.worldObj!=null && !this.worldObj.isRemote)
    ...
    this.broadcastWork(AWCoreStatics.npcAISearchRange);    
    }
  }
...
protected void validateWorkers()
  ...
  this.workerValidationDelayTicks = AWCoreStatics.npcAITicks * 10;
  Iterator<IWorker> workIt = this.workers.iterator();
  IWorker npc = null;
  while(workIt.hasNext())
    {
    npc = workIt.next();
    if(npc==null || npc.isDead() || npc.getDistanceFrom(xCoord, yCoord, zCoord)>AWCoreStatics.npcAISearchRange)
      {      
      workIt.remove();
      continue;
      }
...
public void broadcastWork(int maxRange)
  ...
  this.broadcastDelayTicks = AWCoreStatics.npcAITicks * 10;
  if(!this.isWorking)
    {
    return;
    }
