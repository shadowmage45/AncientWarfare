...
//  Config.logDebug("Setting structure to edit");
  ...
  if(this.serverStructure==null)
    {
    AWLog.logError("Severe error attempting to set structure for editing: "+this.currentEditingStructure);
  ...
  try
    ...
    {
    AWLog.logError("Error sending template to client");
    e.printStackTrace();
    } 
  catch (IOException e)
    {
...
private void saveTemplateServer(ProcessedStructure newStruct)
  ...
  if(!StructureExporter.writeStructureToFile(newStruct, newStruct.filePath, true))
    {
    AWLog.logError("Error exporting structure: "+newStruct.name);
    }
