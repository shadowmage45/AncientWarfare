...
public StructureBuilder(World world, ProcessedStructure struct, int facing, BlockPosition hit)
  {
  this(struct, facing, hit);
  this.setWorld(world);
  }

protected StructureBuilder(ProcessedStructure struct, int facing, BlockPosition hit)
  {
  this.struct = struct;
  this.buildPos = hit;  
  this.facing = facing;
...
protected void setBlock(World world, int x, int y, int z, int id, int meta)
  ...
  if(id<0)
    {
    AWLog.logError("Attempt to set block to invalid id: "+x+","+y+","+z+","+"::"+id+"-"+meta + " from structure: "+struct.name);
  ...
  if(world.blockExists(x, y, z))
    ...
    if(isWorldGen)
      ...
      try
        ...
        {
        AWLog.logError("caught exception while attempting to place block: "+x+","+y+","+z+" :: "+id+"-"+meta);
        }
      }
    else
      {
