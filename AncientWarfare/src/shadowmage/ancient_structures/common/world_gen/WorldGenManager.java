...
//    Config.logDebug("underground structure--invalid topBlock");
  ...
  if(y-size <=1)
    {
    AWLog.logDebug("site rejected by structure, too low");
    return false;
    }
...
//    Config.logDebug("underground structure rejected build site");
  ...
  AWLog.log("Ancient Warfare generating structure at: "+x+","+y+","+z +" :: "+struct.name);
...
public boolean attemptPlacementSurface(World world, int x, int y, int z, int face, ProcessedStructure struct, Random random)
  ...
  if(y==-1)
    {
    AWLog.logDebug("invalid topBlock");
    return false;
    }
  int size = struct.ySize - (struct.ySize-struct.verticalOffset);
  if(y-size <=1)
  ...
  if(!struct.canGenerateAtSurface(world, hit.copy(), face, struct))
    {
    AWLog.logDebug("site rejected by structure: "+struct.name);
    return false;
    }  
  
  if(this.checkBBCollisions(world, struct, hit, face, x/16, z/16))
    {
    AWLog.logDebug("site rejected due to structure overlap");
    return false;
    }
  AWLog.log("Ancient Warfare generating structure at: "+x+","+y+","+z +" :: "+struct.name);
//  BuilderInstant builder = new BuilderInstant(world, struct, face, hit);
...
public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
  ...
  if(!WorldGenStructureManager.instance().isValidDimension(dim))
    {
    AWLog.logDebug("invalid dimension for generation");
    return;
    }
...
//    Config.logDebug("Exit for early random chance check");
  ...
  if(spawnDist< 12 *16 )
    {    
    AWLog.logDebug("structure gen exit for too close to spawnPos");
    return;
    }
  ...
  if(struct!=null)
    ...
    if(random.nextFloat() < valPercent)
      {
      AWLog.logDebug("Exit for value ratio check");
      return;
      }        
    /**
    ...
    if(placed)
      {   
      AWLog.logDebug("generated : "+struct.name);
      ...
      {
      AWLog.logDebug("placement fail");
      }
    }
  else
    {
    AWLog.logDebug("exit for null structure");
    }
  }
