...
public void addEntry(WorldGenStructureEntry entry)
  ...
  if(!structureEntries.containsKey(entry.name))
    {
    AWLog.logDebug("adding structure to world gen: "+entry.name);
    structureEntries.put(entry.name, entry);
    }
  else
    {
    AWLog.logError("Attempt to register duplicate structure name for world-generation. name: "+entry.name);
    return;
    } 
  }
...
//  Config.logDebug("Attempting to add world gen entry for structure: "+ent.name);
  ...
  if(struct!=null)
    ...
    if(biomesNotIn!=null && biomesOnlyIn!=null)
      {
      AWLog.logError("Error detected in a structure template:  it has both exclusive and inclusive biome lists.  Please use only one or the other.  Structure name: "+struct.name);
      return;
      }    
    if(biomesNotIn!=null)
      {
    ...
    {
    AWLog.logDebug("Null structure returned when trying to add world gen entry for structure: "+ent.name);
    }
  }
...
public void loadConfig(String pathName)
  ...
  try
    ...
    if(!configFile.exists())
      ...
      AWLog.log("AWWorldGen.cfg could not be located, creating default file.");      
      if(!configFile.exists())
        {
        AWLog.logError("Error creating or accessing world gen configuration file, world gen is not enabled");
        return;
        }
      }
    else if(AWStructureStatics.updatedVersion && AWStructureStatics.autoExportOnUpdate)
      {
      ...
      copyDefaultFile(fileName);
      AWLog.log("Updated mod version detected, re-exporting AWWorldGen.cfg as per config settings.");
      AWLog.log("Your old config has been saved as AWWorldGen.old.");
      if(!configFile.exists())
        {
        AWLog.logError("Error creating or accessing world gen configuration file, world gen is not enabled");
        return;
    ...
    AWLog.log("Added "+addedEntries + " world gen structure entries.");
    }
  catch(IOException e)
    {
    AWLog.logError("Error while loading world_gen configuration file, no structures will be generated");
    e.printStackTrace();
    }
  catch(NumberFormatException e)
    {
    AWLog.logError("Improperly formatted world gen config file, could not parse a number value");
    e.printStackTrace();
    } 
  }
...
public void saveConfig()
  ...
  if(this.configFile==null)
    {
    AWLog.logError("Null file reference when attempting to save world-gen config to disk");
    return;
    }
  try
    {
    ...
    {
    AWLog.logError("Error while attempting to save world-gen config to disk");
    }
  }
...
private void parseConfig(Iterator<String> it)
  ...
  if(this.validDimensions!=null && this.invalidDimensions !=null)
    {
    AWLog.logError("Invalid World Gen configuration detected.  Entry detected for both valid and invalid dimensions.  You may specify one OR the other, but may not specify both.");
    }
  }
...
private WorldGenStructureEntry parseEntry(Iterator<String> it)
  ...
  if(name.equals("") || weight == 0 || value == -1)
    {
    AWLog.logError("Improperly formatted structure in world gen config file");
    return null;
    }
  if(mC >=0 && cB<0)
    {
